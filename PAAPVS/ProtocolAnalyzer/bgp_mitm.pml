...
/*
此版本为加入攻击者版本。在路由器0和路由器1之间加入一个攻击者，验证BGP是否能应对
...
DONE0:
	i=0;
	doneFlag[0]=true;
	/*判断每个路由表中是否有不存在的下一跳地址201或者211*/
	do
	::(i<6)->
		assert(BGP_table_route0[i].attribute[1]!=201 && BGP_table_route0[i].attribute[1]!=211);
		i++;
	::else->break;
	od
}
...
DONE1:
	...
	do
	::(i<6)->
		assert(BGP_table_route1[i].attribute[1]!=201 && BGP_table_route1[i].attribute[1]!=211);
		i++;
	::else->break;
	od
}
...
DONE2:
	...
	do
	::(i<6)->
		assert(BGP_table_route2[i].attribute[1]!=201 && BGP_table_route2[i].attribute[1]!=211);
		i++;
	::else->break;
	od
}

proctype attack(chan in1,in2,out1,out2){
	Msg rece;
	/*
	拦截路由器0和1发送的任何消息，然后伪装成路由器1发给路由器0；
	伪装成路由器0发给路由器1
	*/
	do
	/*OPEN、KEEPALIVE、ACK报文不做修改*/
	::in1?OPEN(rece)->out2!OPEN(rece);
	::in2?OPEN(rece)->out1!OPEN(rece);
	::in1?KEEPALIVE(rece)->out2!KEEPALIVE(rece);
	::in2?KEEPALIVE(rece)->out1!KEEPALIVE(rece);
	::in1?ACK(rece)->out2!ACK(rece);
	::in2?ACK(rece)->out1!ACK(rece);
	::in1?UPDATE(rece)->
	/*收到来自路由器0的消息后，将下一跳的地址全部设置成不存在的值211*/
		rece.NEXT_HOP=211;  
		out2!UPDATE(rece);
	::in2?UPDATE(rece)->
	/*收到来自路由器1的消息后，将下一跳的地址全部设置成不存在的值201*/
		rece.NEXT_HOP=201;
		out1!UPDATE(rece);
	::timeout->goto DONE3;
	od
DONE3:doneFlag[3]=true;
}
...
init{
	atomic
	{
		run route0(r0_4[1],r0_2[1],r0_4[0],r0_2[0]);
		run route1(r1_4[1],r1_2[1],r1_4[0],r1_2[0]);
		run route2(r1_2[0],r0_2[0],r1_2[1],r0_2[1]);
		run attack(r0_4[0],r1_4[0],r0_4[1],r1_4[1]);
		run timer()
	}
}
