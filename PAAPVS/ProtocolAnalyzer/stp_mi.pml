...
原理：由于STP协议没有密码学原理，
没有身份验证机制，
且自身缺陷使得交换机会尝试尽可能多的去处理BPDU报文。
攻击设备可与要攻击的网络设备直连。
攻击设备可通过发送大量BPDU报文导致交换机CPU占用率升高，达成使该交换机处于拒绝服务状态的目标。
结果：交换机拒绝服务。
*/
/*用于超时计时器的标志位*/
byte timeoutSA = 0;
...
mtype = {CON_BPDU,DATA}
/*通道*/
chan r0_1[2]=[10]of{mtype,Msg}
chan r0_2[2]=[10]of{mtype,Msg}
chan r1_2[2]=[10]of{mtype,Msg}
init{
	atomic{
		run Mybridge0(r0_1[1],r0_2[1],r0_1[0],r0_2[0]);
		run Mybridge1(r0_1[0],r1_2[1],r0_1[1],r1_2[0]);
...
proctype Mybridge0(chan in1,in2,out1,out2){
	...
	/*信息初始化*/
	Bridge bridge00
	bridge00.Bridge_Identifier=0
	bridge00.Designated_Root=0
	bridge00.Root_Path_Cost=0
	bridge00.Root_Port=3
	/*端口ID=0的初始化*/
	bridge00.ports[0].Port_Identifier=0
	bridge00.ports[0].Bridge_Identifier=0
	bridge00.ports[0].Path_Cost=2
	bridge00.ports[0].Designated_Root=0 
	bridge00.ports[0].Designated_Cost=0
	bridge00.ports[0].Designated_Bridge=0
	...
	bridge00.ports[1].Bridge_Identifier=0
	bridge00.ports[1].Path_Cost=6
	bridge00.ports[1].Designated_Root=0 
	bridge00.ports[1].Designated_Cost=0 
	bridge00.ports[1].Designated_Bridge=0
	...
	Msg smsg1;
	Msg rmsg;
	smsg0.Root_Identifier=0 
	smsg0.Root_Path_Cost=0  
	smsg0.Bridge_Identifier=0 
	smsg0.Port_Identifier=0 	
	smsg1.Root_Identifier=0 
	smsg1.Root_Path_Cost=0  
	smsg1.Bridge_Identifier=0 
...
proctype Mybridge1(chan in1,in2,out1,out2){
	...
	/*信息初始化*/
	Bridge bridge01
	bridge01.Bridge_Identifier=1
	bridge01.Designated_Root=1
	bridge01.Root_Path_Cost=0
	bridge01.Root_Port=3 
	/*端口ID=0的初始化*/
	bridge01.ports[0].Port_Identifier=0
	bridge01.ports[0].Bridge_Identifier=1
	bridge01.ports[0].Path_Cost=2
	bridge01.ports[0].Designated_Root=1  
	bridge01.ports[0].Designated_Cost=0 
	bridge01.ports[0].Designated_Bridge=1 
	...
	bridge01.ports[1].Bridge_Identifier=1
	bridge01.ports[1].Path_Cost=3
	bridge01.ports[1].Designated_Root=1  
	bridge01.ports[1].Designated_Cost=0  
	bridge01.ports[1].Designated_Bridge=1 
...
proctype Mybridge2(chan in1,in2,out1,out2){
	...
	/*信息初始化*/
	Bridge bridge02; 
	bridge02.Bridge_Identifier=2
	bridge02.Designated_Root=2
	bridge02.Root_Path_Cost=0
	bridge02.Root_Port=3 
	/*端口ID=0的初始化*/
	bridge02.ports[0].Port_Identifier=0
	bridge02.ports[0].Bridge_Identifier=2
	bridge02.ports[0].Path_Cost=6
	bridge02.ports[0].Designated_Root=2  
	bridge02.ports[0].Designated_Cost=0 
	bridge02.ports[0].Designated_Bridge=2 
	...
	bridge02.ports[1].Bridge_Identifier=2
	bridge02.ports[1].Path_Cost=3
	bridge02.ports[1].Designated_Root=2  
	bridge02.ports[1].Designated_Cost=0  
	bridge02.ports[1].Designated_Bridge=2 
...
proctype attacker(chan out){
	...
	byte i=0;
	/*往桥0和1的通道中不停的注入错误信息*/
	rmsg.Root_Identifier = 9
	rmsg.Root_Path_Cost = 0
	rmsg.Bridge_Identifier = 9
	rmsg.Port_Identifier = 0
	do
		::(i<10)->
			out!CON_BPDU(rmsg)
			i++
