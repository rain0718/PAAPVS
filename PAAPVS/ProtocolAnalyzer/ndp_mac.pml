...
/*
此版本为加入攻击者版本；
该攻击者可以伪造NS/NA报文进行欺骗，与IPv4中的ARP欺骗类似，通过发送包含虚假链路地址
的邻居请求报文NS报文或邻居通告报文NA，改变被攻击者的邻居缓存，致使被攻击者将报文转发到
虚假的链路层地址，或者破坏被攻击节点和其他所有节点的正常通信。（伪造MAC地址的地址解析攻击）
*/
/*IPv6头部*/
typedef IPv6_Header { 
	byte version;/*版本号*/
...
{
	/*
	获取网络内其他主机发送给h0的报文，然后进行伪造NA报文欺骗,使得h0缓存的mac地址无效，
	无法与目标主机进行通信
	*/
	Msg rece;
	byte count=0;
	...
	::in1?NS(0,rece)->out2!NS(0,rece);count++;
	::in1?NA(1,rece)->
		rece.target_link_layer_address=211;
		out2!NA(1,rece);
		count++;
	::(count==6)->goto DONE4;
	od
