/*
TFTPv5 考虑LTL和assert，其他功能均已完善，无攻击者
*/
#define MAX = 4
...
mtype={RRQ,WRQ,DATA,ACK,ERROR}
	chan s_c=[10]of{mtype,byte,Msg};/*报文类型，文件编号，报文内容*/
...
S1:			if
				::(scount>=2)->out!ERROR(st,sSmesg);goto sDone
				::else ->
					do
							::gbg!DATA(st,sSmesg);scount=scount+1;goto S1
							::out!DATA(st,sSmesg);scount=0;break
							::out!DATA(st,sSmesg);scount=0;break
							::out!DATA(st,sSmesg);scount=0;break
					od
			fi
...
S3:			if
			...
			::else ->
				...
				::out!ACK(st,sSmesg);scount=0;break
				::out!ACK(st,sSmesg);scount=0;break
				::out!ACK(st,sSmesg);scount=0;break
				od
			fi
...
//ltl p1 {([](correctPacket-> <>(ACKSent||ErrorSent)))}
//ltl p1 {(c_s?[DATA])-> <>(s_c![ACK]||s_g![ACK])}
...
ltl p5 {q7-> <>(client@cDone)}
 ...
 /*ltl p1 {([]<>(((in?DATA(stt,sRmesg)&&(sRmesg.Num==ss))||(in?ACK(stt,sRmesg)&&(sRmesg.Num==ss))))->[]<>((out!ACK(st,sSmesg)||gbg!ACK(st,sSmesg))||(out!DATA(st,sSmesg)||gbg!DATA(st,sSmesg)||(server@sDone))))}
*/
