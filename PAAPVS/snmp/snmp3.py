...
def get_case_info(target, fuzz_data_logger, session, sock, *args, **kwargs):
    fuzz_data_logger.log_info("SNMP Encoding Identifier: \n" + str(session.nodes[1].names["SNMP Encoding Identifier"]._value))
    fuzz_data_logger.log_info("SNMP Community: \n" + str(session.nodes[1].names["SNMP Community"]._value))
    fuzz_data_logger.log_info("Request-Id: \n" + str(session.nodes[1].names["Request-Id"]._value))
    fuzz_data_logger.log_info("Oid: \n" + str(session.nodes[1].names["Oid"]._value))
    fuzz_data_logger.log_info("Type Null: \n" + str(session.nodes[1].names["Type Null"]._value))
    fuzz_data_logger.log_info("Error Index: \n" + str(session.nodes[1].names["Error Index"]._value))
...
def main():
    ...
    with s_block('SNMP Header'):
        s_string(snmpiden_data, name = 'SNMP Encoding Identifier')
        #s_bit_field(snmpiden_data, width = 8, endian = '>', full_range = False, name = 'SNMP Encoding Identifier')
    ...
    with s_block(name = 'Get/Set Header'):
        ...
        s_string(errindex_data, name = 'Error Index')
    ...
    with s_block(name = 'Variable-Bindings'):
        ...
        s_string(valuenull_data, name = 'Type Null')
