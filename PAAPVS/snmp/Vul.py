...
from snmp_widget import Ui_Form
import sys
...
class Vul(QWidget,Ui_Form):
    ...
    def __init__(self):
        ...
        self.version = self.config.get('snmp', 'version')
        self.sofe_name = self.config.get('snmp', 'name')
    ...
    def start_slot(self):
        ...
        self.community_agv = self.com_edit.text()
        self.snmp_start()

    def snmp_start(self):
        self.thread = QThread()
        def run_snmp():
            shell_cmd = 'python snmp.py {} {} {}'.format(self.host_agv, self.port_agv,self.community_agv)
            # print(shell_cmd)
